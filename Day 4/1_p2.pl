...
foreach my $line ( @sort ){
    if( $line =~ /Guard #([0-9]+) / ) {
        $id = $1;
        $guards{$id} //= { maxCount => 0, maxMin => 0, sleepMins => 0, mins => []};
    }
    elsif( $line =~ /:([0-9]{2})\].*falls/) {
        $sleep = $1;
        $guards{$id}->{mins}->[$sleep] //= 0;
    }
    elsif( $line =~ /:([0-9]{2})\].*wakes/) {
        $wake = $1;
        my $numMins = $wake - $sleep;
        $guards{$id}->{sleepMins} += $numMins;
        for ( my $j = $sleep; $j < $wake; $j++ ) {
            $guards{$id}->{mins}->[$j]++;
            if( $guards{$id}->{maxCount} < $guards{$id}->{mins}->[$j] ) {
                $guards{$id}->{maxCount} = $guards{$id}->{mins}->[$j];
                $guards{$id}->{maxMin} = $j;
            }
            if( $maxCount < $guards{$id}->{maxCount} ) {
                $maxCount = $guards{$id}->{maxCount};
                my $maxMin = $guards{$id}->{maxMin};
                $maxId = $id;
                say "$id - $maxCount - $maxId - $maxMin";
            }
        }
    }
}

#say Dumper($guards{$maxId});
say Dumper(\%guards);
say $maxId .'-'.$guards{$maxId}->{maxMin};
say $maxId * $guards{$maxId}->{maxMin};